  ...
  computed: {
    ...
    computeProgramHours() {
      this.project_logs.forEach((val, i) => {
        ...
        val.project_logs.forEach((value, index) => {
          ...
          let next_line_remarks_date = moment(
            new Date(this.project_logs[i].project_logs[index].remarks_date),
            "YYYY-MM-DD"
          );
          let next_line_remarks_time = this.project_logs[i].project_logs[index]
            .remarks_time;
          let next_line_remarks_datetime = moment(
            new Date(
              this.project_logs[i].project_logs[index].remarks_date +
                " " +
                this.project_logs[i].project_logs[index].remarks_time
            ),
            "YYYY-MM-DD"
          );
          ...
          let prev_line_remarks_time = this.project_logs[i].project_logs[index]
            .remarks_time;
          let next_line_status = this.project_logs[i].project_logs[index]
            .status;
          let prev_line_status = this.project_logs[i].project_logs[index]
            .status;
          ...
          let mins = 0;
          let last_index = this.project_logs[i].project_logs.length - 1;

          // get the previous status if index is greater than 0
          if (index > 0) {
            prev_line_status = this.project_logs[i].project_logs[index - 1]
              .status;
            prev_line_remarks_date = moment(
              new Date(
                this.project_logs[i].project_logs[index - 1].remarks_date
              ),
              "YYYY-MM-DD"
            );
            prev_line_remarks_time = this.project_logs[i].project_logs[
              index - 1
            ].remarks_time;
            prev_line_remarks_datetime = moment(
              new Date(
                this.project_logs[i].project_logs[index - 1].remarks_date +
                  " " +
                  this.project_logs[i].project_logs[index - 1].remarks_time
              ),
              "YYYY-MM-DD"
            );
          }
          ...
          if (this.project_logs[i].project_logs.length > index + 1) {
            next_line_status = this.project_logs[i].project_logs[index + 1]
              .status;
            ...
            next_line_remarks_time = this.project_logs[i].project_logs[
              index + 1
            ].remarks_time;
